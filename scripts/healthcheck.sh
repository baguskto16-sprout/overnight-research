...
# and login profile are not sourced). Must include node — claude CLI shells out
# to `env node` and exits with "node: No such file or directory" otherwise.
...
if [ -s "$NVM_DIR/nvm.sh" ]; then
  ...
  # nvm.sh in non-interactive mode doesn't auto-activate; pick latest installed.
  NODE_BIN=$(ls -d "$NVM_DIR"/versions/node/*/bin 2>/dev/null | sort -V | tail -1)
  [ -n "$NODE_BIN" ] && export PATH="$NODE_BIN:$PATH"
fi
# Fallback: Homebrew node@22 keg path if nvm not present.
